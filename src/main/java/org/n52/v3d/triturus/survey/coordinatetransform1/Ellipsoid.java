...
public class Ellipsoid {
    ...
    // geogr. Breite Lotfu�punkt
    private double gE0;
    private double gF2;
    private double gF4;
    private double gF6;

    private double[] en = new double[5]; // Meridianbogenabst�nde
...
//	pj_errno = -17;
    ...
    public double getQuerkruemmungshalbmesser(double lat) {
        return powA / Math.sqrt(
            ...
            powB * Math.pow (Math.sin (lat), 2.0d)); //Querkr�mmungshalbmesser // ??? 2.0l
    }

    public double getPolkruemmungshalbmesser() {
        //Kr�mmungshalbmesser am Pol (lat = 90�):
        return powA / b;
    }

    //Kr�mmungshalbmesser der Hauptvertikalen (f�r Molodensky)
    public double getRN(double lat) {
        return a / Math.sqrt(1.0d/*l*/ - powE1 * Math.pow(Math.sin(lat), 2.0d/*l*/));
    }

    //Kr�mmungshalbmesser im Hauptmeridian (f�r Molodensky)
    ...
    public double[] geocentricToEll(double x, double y, double z, double[] out) throws GeographicTransformException {
        ...
        //alternative Funktion f�r die H�he (Transformation of GPS Results)
    ...
    private void init() {
        ...
        //Konstanten f�r Meridianbogenl�nge
