...
/**
 * Class to hold indexed TIN-structures (<tt>VgTIN</tt> specialization). 
 * The vertices that are part of the indexed TIN are numbered, the triangles 
 * (faces) can be referenced by these index numbers.
 *
...
{
	/**
	 * returns the vertex-indices if the i-th triangle (face) inside the TIN.
	 * The assertion 0 &lt;= i &lt; <tt>this.numberOfTriangles()</tt> must hold;
